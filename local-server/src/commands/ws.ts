...
const wsCommand = async (args: string[]) => {
  const linkValue = args[3]

  if (!linkValue) {
    console.error('Link is required for WS. Usage: a2rok ws <link>')
    console.error('Example: a2rok ws ws://localhost:8080')
    process.exit(1)
  }

  // Validate that the link starts with ws://
  if (!linkValue.startsWith('ws://')) {
    console.error('Link must start with ws:// for WebSocket tunneling')
    console.error('Example: a2rok ws ws://localhost:8080')
    process.exit(1)
  }
  ...
  await connectWebSocket(0, session.token, 'ws', linkValue)
  ...
  setInterval(() => {
    ...
    console.log(
      '\x1b[1m\x1b[36m║                              A2Rok (WS)                               ║\x1b[0m',
    )
    console.log(
      '\x1b[1m\x1b[36m╚══════════════════════════════════════════════════════════════════════════════╝\x1b[0m',
    )
    ...
    console.log(
      `\x1b[37mForwarding:\x1b[0m \x1b[34mws://${domainMetadata.domain}.a2rok-server.harshkeshri.com/ws-tunnel\x1b[0m \x1b[90m->\x1b[0m \x1b[34m${linkValue}\x1b[0m`,
    )
    console.log('')
    ...
    console.log('\x1b[1m\x1b[33mRecent WebSocket Messages:\x1b[0m')
