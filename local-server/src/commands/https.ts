...
const httpsCommand = async (args: string[]) => {
  const linkValue = args[3]

  if (!linkValue) {
    console.error('Link is required for HTTPS. Usage: a2rok https <link>')
    console.error('Example: a2rok https https://my-domain.com')
    process.exit(1)
  }
  ...
  await connectWebSocket(0, session.token, 'https', linkValue)
  ...
  setInterval(() => {
    ...
    console.log(
      '\x1b[1m\x1b[36m║                              A2Rok (HTTPS)                              ║\x1b[0m',
    )
    console.log(
      '\x1b[1m\x1b[36m╚══════════════════════════════════════════════════════════════════════════════╝\x1b[0m',
    )
    ...
    console.log(
      `\x1b[37mForwarding:\x1b[0m \x1b[34mhttps://${domainMetadata.domain}.a2rok-server.harshkeshri.com\x1b[0m \x1b[90m->\x1b[0m \x1b[34m${linkValue}\x1b[0m`,
    )
    console.log('')
